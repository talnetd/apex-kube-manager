<script lang="ts">
  ...
  import { configmaps, selectedNamespace, currentContext, loadConfigMaps, isLoading } from '../../stores/kubernetes';
...
<div class="h-full flex flex-col overflow-hidden">
  <div class="p-6 pb-4">
    <div class="flex items-center justify-between mb-4">
      <div>
        <h1 class="text-2xl font-semibold text-text-primary">ConfigMaps</h1>
        <p class="text-text-muted mt-1">Non-confidential configuration data</p>
      </div>
      <button onclick={() => loadConfigMaps($selectedNamespace)} class="flex items-center gap-2 px-4 py-2 bg-bg-tertiary border border-border-subtle rounded-lg text-text-primary hover:border-accent-primary transition-colors">
