...
<div class="h-full flex flex-col overflow-hidden">
  ...
  <div class="p-6 pb-4">
    <div class="flex items-center justify-between mb-4">
      <div>
        <h1 class="text-2xl font-semibold text-text-primary">StatefulSets</h1>
        <p class="text-text-muted mt-1">Manage stateful applications with persistent storage</p>
      </div>
      <button
        onclick={() => loadStatefulSets($selectedNamespace)}
    ...
    <table class="w-full">
      <thead>
        <tr class="text-left border-b border-border-subtle">
          ...
          <th class="pb-3 text-xs text-text-muted uppercase tracking-wide font-medium">Service</th>
          ...
          <tr class="border-b border-border-subtle/50 hover:bg-bg-secondary transition-colors">
            <td class="py-3 pr-4">
              <div class="flex items-center gap-2">
                ...
                <span class="text-text-primary font-medium">{sts.name}</span>
              </div>
            </td>
            <td class="py-3 pr-4">
